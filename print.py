def determine(highest_categories, highest_overhead, coh_result, pl_result):
    coh_print_key = 'determine_print'
    pl_print_key = 'pl_determine_print'

    for entry in coh_result:
        if entry[0] == coh_print_key:
            coh_determine = entry[1]
            break
    for entry in pl_result:
        if entry[0] == pl_print_key:
            pl_determine = entry[1]
            break
    
    if coh_determine == '1' and pl_determine == '1':
        Scenario1(highest_categories, highest_categories, coh_result, pl_result)
    elif coh_determine == '2' and pl_determine == '2':
        Scenario2(highest_categories, highest_overhead, coh_result, pl_result)
    else:
        Scenario3(highest_categories, highest_overhead, coh_result, pl_result)
